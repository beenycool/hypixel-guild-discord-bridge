...
export default class DuelsClassic extends ChatCommandHandler {
  constructor() {
    super({
      triggers: ['duels-classic', 'dclassic', 'classic'],
      description: "Returns a player's Classic Duels stats",
      example: `duels-classic %s`
    })
  }
  ...
  async handler(context: ChatCommandContext): Promise<string> {
    ...
    const modeData = (stats as unknown as Record<string, unknown>).classic
    if (!modeData || typeof modeData !== 'object') {
      return `${givenUsername} has no Classic Duels stats.`
    }
    ...
    return (
      `[Classic] [${this.formatDivision(division)}] ${givenUsername} ` +
