...
export default class DuelsBoxing extends ChatCommandHandler {
  constructor() {
    super({
      triggers: ['duels-boxing', 'dboxing', 'boxing'],
      description: "Returns a player's Boxing Duels stats",
      example: `duels-boxing %s`
    })
  }
  ...
  async handler(context: ChatCommandContext): Promise<string> {
    ...
    const modeData = (stats as unknown as Record<string, unknown>).boxing
    if (!modeData || typeof modeData !== 'object') {
      return `${givenUsername} has no Boxing Duels stats.`
    }
    ...
    return (
      `[Boxing] [${this.formatDivision(division)}] ${givenUsername} ` +
