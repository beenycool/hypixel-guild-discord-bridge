...
export default class DuelsNoDebuff extends ChatCommandHandler {
  constructor() {
    super({
      triggers: ['duels-nodebuff', 'dnodebuff', 'nodebuff'],
      description: "Returns a player's NoDebuff Duels stats",
      example: `duels-nodebuff %s`
    })
  }
  ...
  async handler(context: ChatCommandContext): Promise<string> {
    ...
    const modeData = (stats as unknown as Record<string, unknown>).nodebuff
    if (!modeData || typeof modeData !== 'object') {
      return `${givenUsername} has no NoDebuff Duels stats.`
    }
    ...
    return (
      `[NoDebuff] [${this.formatDivision(division)}] ${givenUsername} ` +
