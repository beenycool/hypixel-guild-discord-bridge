...
export default class DuelsUHC extends ChatCommandHandler {
  constructor() {
    super({
      triggers: ['duels-uhc', 'duhc', 'uhc'],
      description: "Returns a player's UHC Duels stats",
      example: `duels-uhc %s`
    })
  }
  ...
  async handler(context: ChatCommandContext): Promise<string> {
    ...
    const modeData = (stats as unknown as Record<string, unknown>).uhc
    if (!modeData || typeof modeData !== 'object') {
      return `${givenUsername} has no UHC Duels stats.`
    }
    ...
    return (
      `[UHC] [${this.formatDivision(division)}] ${givenUsername} ` +
