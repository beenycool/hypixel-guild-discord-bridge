...
export default class DuelsCombo extends ChatCommandHandler {
  constructor() {
    super({
      triggers: ['duels-combo', 'dcombo', 'combo'],
      description: "Returns a player's Combo Duels stats",
      example: `duels-combo %s`
    })
  }
  ...
  async handler(context: ChatCommandContext): Promise<string> {
    ...
    const modeData = (stats as unknown as Record<string, unknown>).combo
    if (!modeData || typeof modeData !== 'object') {
      return `${givenUsername} has no Combo Duels stats.`
    }
    ...
    return (
      `[Combo] [${this.formatDivision(division)}] ${givenUsername} ` +
