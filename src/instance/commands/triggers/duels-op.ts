...
export default class DuelsOP extends ChatCommandHandler {
  constructor() {
    super({
      triggers: ['duels-op', 'dop', 'op'],
      description: "Returns a player's OP Duels stats",
      example: `duels-op %s`
    })
  }
  ...
  async handler(context: ChatCommandContext): Promise<string> {
    ...
    const modeData = (stats as unknown as Record<string, unknown>).op
    if (!modeData || typeof modeData !== 'object') {
      return `${givenUsername} has no OP Duels stats.`
    }
    ...
    return (
      `[OP] [${this.formatDivision(division)}] ${givenUsername} ` +
